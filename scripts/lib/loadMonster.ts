...
import { ArtifactDetail, GearSet, Monster, RelicDetail, RtaEntry, RuneDetail, SiegeTeam } from '../../src/types';
...
export function loadBoxMonster(exportPath: string, monsterName: string): LoadedMonster {
  ...
  const { runes: allRunes, artifacts: allArtifacts, relics: allRelics } = parseAccountInventory(data);
  return { unitId: best.unitId, com2usId: best.com2usId, monsterName, gear: best.gear!, allRunes, allArtifacts, allRelics };
}
...
export function loadRtaMonster(exportPath: string, monsterName: string): LoadedMonster {
  ...
  return { unitId: -1, com2usId: best.com2usId, monsterName, gear: best.gear!, allRunes, allArtifacts, allRelics };
}
...
export function loadSiegeMonster(args: DeckMonsterArgs): LoadedMonster {
  ...
  const { artifacts: allArtifacts, relics: allRelics } = parseAccountInventory(parseAccountSource(readFileSync(args.exportPath, 'utf8'))!);
  ...
  return { unitId: -1, com2usId, monsterName: args.monsterName, gear, allRunes, allArtifacts, allRelics };
}
