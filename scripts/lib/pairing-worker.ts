...
import { SearchParams, Bucket, prepareSearch, pairBuckets } from '../../src/lib/runeBuildOptim';
...
if (!prepared) {
  ...
  const result = drain(pairBuckets(prepared, data.bucketASlice, data.bucketsB));
