...
function findRank(combos: HalfCombo[], targetIds: Set<number>) {
...
const rankA = findRank(combosA, targetIdsA);
const rankB = findRank(combosB, targetIdsB);
console.log(`Demi-build A cible : ${rankA.rank ? `rang #${rankA.rank}/${rankA.total} (retenu)` : `❌ ABSENT (${rankA.total} retenus au total)`}`);
console.log(`Demi-build B cible : ${rankB.rank ? `rang #${rankB.rank}/${rankB.total} (retenu)` : `❌ ABSENT (${rankB.total} retenus au total)`}`);
