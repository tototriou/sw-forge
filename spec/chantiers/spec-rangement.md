...
| C2 `chantier rafraichir` — les notes se tirent aussi (fusion `main` documentaire → branche du chantier, miroir vers le worktree de code, refus si notes inédites) | J (effort moyen) | exécuté | `ff34b77`, `786cbab` ; 38 tests ; preuve réelle : audit 45 → 72 fichiers, reçu valide ; rejoué sur orchestration : refus « livrer d'abord » puis conflit annulés proprement — **défaut** : la marche à suivre imprimée (fusion à la main) est ensuite refusée comme « avance indépendante », 2026-09-17 |
| C2 bis `rafraichir` reprend une fusion de `main` résolue à la main (premier parent = révision attendue, second = `main`) ; test de reprise ; preuve réelle sur orchestration | M (Sonnet, effort moyen) | exécuté | `d841537` ; test de reprise KO avant / 129 après ; orchestration : reprise `a0876f8 → 1dd25af`, `verifier` 8/8, second `rafraichir` « déjà à jour » (rejouable), 2026-09-17 |
| C3 fusion sur `main` : merge d'`origin/main`, fix `tsc` des fixtures speed-tune (PR #9), entrée unique 1.13.0, `package.json`, PR, tag, release | — | exécuté | PR #10, merge `838641e`, tag `v1.13.0`, release publiée, 2026-09-17 |
| C4 après fusion : `chantier fermer` ×3, avance de `forge/implementation-relique` sur `main` + `rafraichir`, hook `Read` câblé, `chantier installer` | — | exécuté | `installer` @ `838641e` ; hook `Read` câblé (`.claude/settings.json`) ; `spec-rangement`, `audit-degats-conditionnels-etape-2`, `orchestration-parallele` fermés (notes sauvegardées, code intégré dans `main`) ; relique `45f9981 → 838641e` (ff), `rafraichir` sans conflit (73 fichiers), installation sur `main` @ `838641e`, 2026-09-17 |
| C6 le lint connaît les cadrages : nature `CHANTIER`, `chantiers/` exempté du seul plafond fichier, `spec/chantiers/**` dans le périmètre, sous-titres sur les 2 blocs > 100 de ce fichier, skill § F corrigé (retour de C1 : un cadrage privé aurait dû se dire DÉCISION et tenir en 500 lignes) | M (Sonnet, effort moyen) | exécuté | `e680d00` (code, fixtures, B.4/B.5, statuts/sous-titres, réinstallation du hook), skill à part, 2026-09-17 |
| C5 chantier relique : réécriture de `plan-implementation-relique.md` en cadrage (skill `cadrage-chantier`), dans `spec/outils/optimizer/chantiers/` | J (Opus, effort élevé) | exécuté | `chantiers/implementation-relique.md` (804 l., statut CHANTIER en cours, 10 lots dont 5a/5b scindés, D1–D10, T1–T10, critère A/B chiffré en B.6), plan archivé `archive/plan-implementation-relique-2026-09-09.md` ; lint 0 ; code `135c157` (`spec/README.md` § Chantiers), notes `7f6a0c5` puis `b88f02f` (A.5 corrigé à la validation : les trois chantiers voisins sont fermés), docs main `7c3166d` ; revue adversariale (2 tours, session tierce) à faire avant le lot 0, 2026-09-17 |
